...
Selects which face controls the crop when several faces are visible. `Keep
...
`Tracking update interval` limits CPU frame readback and correlation-tracker
...
tracking remains stationary until the error clears the nonlinear band. Residual
integral and low-pass motion are removed without moving the crop, preventing a
stationary subject from causing back-and-forth jitter.
